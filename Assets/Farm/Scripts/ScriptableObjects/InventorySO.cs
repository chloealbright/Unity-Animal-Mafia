...
namespace InventoryCont.Model
{
    ...
    public class InventorySO : ScriptableObject
    {
        ...
        public int ContainsItem(ItemSO item)
        {//GoldShop.cs check if item is in inventory
            int hasItem = 1;
            for (int i = 0; i < inventoryItemStructs.Count; i++)
            {
        ...
        public int RemoveItem(ItemSO item, int quantity)
        {//GoldShop.cs remove item in inventory
            //if item is not stackable, it will add to inventory in the next empty slot
            if (item.IsStackable == false)
            {
                for (int i = 0; i < inventoryItemStructs.Count; i++)
